...
- The M74VHC1GU04DFT1G signal inverter can be tricky to source as it is discontinued (sold out at Digikey for example) so a solution is to switch to a MC74VHC1GU04DF1G (same pinout, same characteristics). **The SC-88A package is equivalent to SOT-353 or SC-70-5 if you search for old new stocks. The package SC−74A, slighly bigger, only one that I found on Aliexpress, can be used too in case of no other choice, it barely fits on the SC-88A footprint (same pinout).**
