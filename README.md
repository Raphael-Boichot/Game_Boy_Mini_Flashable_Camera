...
- The project making was literally plagued on my side. My Digikey order had all components count doubled without any reason (I was the happy buyer of 20 ribbon cables...). But, the signal inverters were also removed from the same order because I've reserved more than the available parts due to this error (And of course I got the information after shipping, by email, while stock of part was OK during checkout). So, I ordered some on Aliexpress, they came in the wrong package (despite being labelled as SC-88A). The platform refuses the refund, I've decided to solder this package anyway. After assembly, the image had random artifacts which came both from a dry solder joint on the signal inverter (due to the wrong package) and issue with capacitors (I though initially that it was a bad FRAM or a fried MAC-GBD as both of them were recycled from another PCB). I had to debug for two days (comprising some chip swap) to find the culprits then re-order a bunch of new capacitors. They arrived during my vacations out of home so impossible to continue the project. Finally it took me more than two months from the first order to the project completion. All of this for a simple fork...
