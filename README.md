...
- I learnt soldering surface mount stuff years ago by just watching Youtube and praticing, so it's not rocket science either. Hardware is the key: choose powerfull enough soldering iron with temperature control (the 2.5€ pen form factor irons sold on Aliexpress are **not** powerfull enough), good solder alloy, good soldering wick, flux in syringe, accurate magnifying device. Best is to start practicing soldering/desoldering with any whatever dead PCB, no need to buy online soldering kits that will end in a drawer.
