...
- The project making was literally plagued on my side: my Digikey order was doubled (and debited twice) without any warning. The signal inverters were completely dismissed from the order because I ordered more than the remaining parts (without warning). I ordered some on Aliexpress, they came in the wrong package (despite being labelled as the good package). Once the PCB was assembled, the image had artifacts which came both from a dry solder joint on the signal inverter (due to the wrong package) and issue with capacitors (I though it was a bad FRAM or a fried MAC-GBD as both were recycled from another PCB). I had to debug for two days (comprising chip swap) to find the culprits then re-order a bunch of new capacitors. They arrived during my vacations out of home so impossible to continue the project. Finally it took me more than two months from the first order to the project completion. So my advice: order EVERYTHING at Mouser if you can.
