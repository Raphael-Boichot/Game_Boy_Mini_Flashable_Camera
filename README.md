...
- The project making was literally plagued on my side. My Digikey order had all components count doubled without any reason. The signal inverters were then removed from the order because I allegedly reserved more than the available parts (I got the information **after shipping**, by email, while stock of parts was still OK during checkout). So no possibility to choose a replacement part without paying shipping fees again. In consequence, I ordered some on Aliexpress with free shipping, they came in the wrong package 3 weeks later (despite being labelled as SC-88A). The platform refuses the refund, I've decided to solder this package anyway. After assembly, the image had random artifacts which came both from a dry solder joint below the signal inverter (due to the wrong package) and issue with capacitors (I though initially that it was a bad FRAM or a fried MAC-GBD as both of them were recycled from another PCB). I had to debug for two more days (comprising some chip swap) to find the culprits and then re-order a bunch of new capacitors. They arrived during my vacations out of home so impossible to continue the project. More than two months pass from the first order to the project completion. All of this for a simple fork that probably nobody will read.
