...
- The project making was literally plagued on my side: my Digikey order was doubled (and paid twice) without any warning. The signal inverters were completely dismissed from the order because I ordered more than the remaining parts (without warning). I ordered some on Aliexpress, they came in the wrong package. Once the PCB was assembled, the image had artifacts which came both from a dry solder joint on the signal inverter and issue with capacitors (I though it was a bad FRAM or a fried MAC-GBD). I had to debug for two days to find the culprits then re-order a bunch of new capacitors. Finally it took me more than two months from the first order to the project completion. So my advice: order EVERYTHING at Mouser if you can.
