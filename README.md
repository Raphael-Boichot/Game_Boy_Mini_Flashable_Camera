...
- You can even completely get rid of the signal inverter by [shorting RAM_CE1 to GND](/Images/Component_placement_alternative.png) with a wire or with a 1k resistor, easier to handle (which means that FRAM is always enabled when camera is on), the camera mod will still works like a charm. My understanding is that the RESET signal is to be used only with a [SRAM / battery management system](https://github.com/MouseBiteLabs/Game-Boy-MBC3-Cartridge/tree/main/Technical) to halt both CPU operation and writing to SRAM when voltage is unstable during switch to the backup battery. I may be wrong of course.
- The schematic only is **open source** and can be edited with Kicad. The author chose to not disclose the PCB layout on purpose. You have to create your own if you want to change something.
