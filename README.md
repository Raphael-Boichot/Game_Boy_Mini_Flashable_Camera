...
- All the 10 nf caps could probably be changed by 100 nf caps if you want to reduce the number of references to order. 100 nf or more is the default cap value for many cart mods on voltage lines. 
...
- You can even completely get rid of the signal inverter by [shorting RAM_CE1 to GND](/Images/Component_placement_alternative.png) with a wire (which means FRAM is always enabled when camera is on), the camera will work like a charm.
