...
- You can even completely get rid of the signal inverter by [shorting RAM_CE1 to GND](/Images/Component_placement_alternative.png) with a wire or with a 1k resistor easier to handle (which means that FRAM is always enabled when camera is on), the camera mod still will work like a charm.
